...
void getDMprof_SIDM(){
  ...
  sprintf(tardir, "/home/storage0/users/zhangxy/CraterII/DMprof-SIDM");
  ...
  for(k=0; k<=2; k++){
    for(n=0; n<=6; n++){
      ...
      sprintf(cmd, "cp %s/DMprof_snap%03d.txt  %s/DMprof-%s-O%d-%03d.txt", fpath, snap[k][n], tardir, type[k], Orb[k], snap[k][n]);
      system(cmd);
