...
void gadgeth2_detect_filetype(FILE *input, char *filename) {
  int32_t first_word;
  SWAP_ENDIANNESS=0;
...
void gadgeth2_read_stride(FILE *input, int64_t p_start, int64_t nelems, int64_t stride, int64_t width, struct particle *p, int64_t offset, int64_t skip, int64_t skip2, char *filename)
{
...
void gadgeth2_extract_header_info(struct gadgeth2_header *header)
{
  int64_t i;
  if (SWAP_ENDIANNESS) {
...
#undef SWAP8
  ...
  TOTAL_PARTICLES = (((int64_t)header->num_total_particles_hw[GH2PT])<<32) +
    (int64_t)header->num_total_particles[GH2PT];
  //According to Matt Becker, LGADGET uses the header fields in a different way:
  if (!strncasecmp(FILE_FORMAT, "LGADGET", 7)) {
    TOTAL_PARTICLES = (((int64_t)header->num_total_particles[GH2PT+1])<<32) +
      (int64_t)header->num_total_particles[GH2PT];
  }
  ...
  if (header->particle_masses[GH2PT] || !PARTICLE_MASS || RESCALE_PARTICLE_MASS) {
    if (!RESCALE_PARTICLE_MASS)
      PARTICLE_MASS = header->particle_masses[GH2PT]*GADGET_MASS_CONVERSION;
    else
  ...
  printf("HALO2 TYPE: %d\n",GH2PT);
  printf("ZXY: HALO 2 number: %d \n",header->num_particles[2]);
  printf("ZXY: header->particle_masses[2] = %f \n",header->particle_masses[2]);
#endif

}

void gadgeth2_rescale_particles(struct particle *p, int64_t p_start, int64_t nelems) {
  int64_t i, j;
  uint32_t id;
...
void load_particles_gadgeth2(char *filename, struct particle **p, int64_t *num_p)
{
  FILE *input;
  char tag[10] = {0};
  struct gadgeth2_header header;
  int64_t total_particles, skip, skip2, halo_particles, i;

  input = check_fopen(filename, "rb");
  gadgeth2_detect_filetype(input, filename);

#define gadgeth2_variant_block(a)		\
  if (GADGET_VARIANT) { \
  ...
  gadgeth2_variant_block("HEAD");
  fread_fortran(&header, GADGET_HEADER_SIZE, 1, input, SWAP_ENDIANNESS);
  gadgeth2_extract_header_info(&header);

  total_particles = 0;
  for (i=0; i<6; i++) total_particles += header.num_particles[i];

  if (1) {
    for (skip=0,i=0; i<GH2PT; i++) skip += header.num_particles[i];
    halo_particles = header.num_particles[GH2PT];
  ...
  *p = (struct particle *)check_realloc(*p, ((*num_p)+halo_particles)*sizeof(struct particle), "Allocating particles.");
  ...
  gadgeth2_variant_block("POS");
  gadgeth2_read_stride(input, *num_p, halo_particles, 3, sizeof(float), 
    *p, (char *)&(p[0][0].pos[0])-(char*)(p[0]), skip, skip2, filename);

  gadgeth2_variant_block("VEL");
  gadgeth2_read_stride(input, *num_p, halo_particles, 3, sizeof(float), 
    *p, (char *)&(p[0][0].pos[3])-(char*)(p[0]), skip, skip2, filename);

  gadgeth2_variant_block("ID");
  gadgeth2_read_stride(input, *num_p, halo_particles, 1, GADGET_ID_BYTES, 
    *p, (char *)&(p[0][0].id)-(char*)(p[0]), skip, skip2, filename);

  gadgeth2_rescale_particles(*p, *num_p, halo_particles);
