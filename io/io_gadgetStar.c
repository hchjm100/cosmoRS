...
void gadgetStar2_detect_filetype(FILE *input, char *filename) {
  int32_t first_word;
  SWAP_ENDIANNESS=0;
...
void gadgetStar2_read_stride(FILE *input, int64_t p_start, int64_t nelems, int64_t stride, int64_t width, struct particle *p, int64_t offset, int64_t skip, int64_t skip2, char *filename)
{
...
void gadgetStar2_extract_header_info(struct gadgetStar_header *header)
{
  int64_t i;
  if (SWAP_ENDIANNESS) {
...
#undef SWAP8
  ...
  TOTAL_PARTICLES = (((int64_t)header->num_total_particles_hw[GSPT])<<32) +
    (int64_t)header->num_total_particles[GSPT];
  //According to Matt Becker, LGADGET uses the header fields in a different way:
  if (!strncasecmp(FILE_FORMAT, "LGADGET", 7)) {
    TOTAL_PARTICLES = (((int64_t)header->num_total_particles[GSPT+1])<<32) +
      (int64_t)header->num_total_particles[GSPT];
  }

  SCALE_NOW = 1;  // header->scale_factor;
  h0 = 0.7;
  Om = 0.31;
  Ol = 0.69;
  // 2023.04.04 ZXY: Revise for non-cosmological simulation

  if (header->particle_masses[GSPT] || !PARTICLE_MASS || RESCALE_PARTICLE_MASS) {
    if (!RESCALE_PARTICLE_MASS)
      PARTICLE_MASS = header->particle_masses[GSPT]*GADGET_MASS_CONVERSION;
    else
  ...
  //PARTICLE_MASS=100000.;
#ifdef DYDEBUG
  printf("\n--------------------- Check header info-------------------\n");
  printf("particle mass 0:%g \n",header->particle_masses[0]);
  printf("particle mass 1:%g \n",header->particle_masses[1]);
  printf("particle mass 2:%g \n",header->particle_masses[2]);
  printf("particle mass 3:%g \n",header->particle_masses[3]);
  printf("particle mass 4:%g \n",header->particle_masses[4]);
  printf("particle mass 5:%g \n",header->particle_masses[5]);
  printf("GADGET_LENGTH_CONVERSION :%f \n",GADGET_LENGTH_CONVERSION);
  printf("Time (scale factor) :%f \n",header->scale_factor);
...
void gadgetStar2_rescale_particles(struct particle *p, int64_t p_start, int64_t nelems) {
  int64_t i, j;
  uint32_t id;
...
void load_particles_gadgetStar2(char *filename, struct particle **p, int64_t *num_p)
{
  FILE *input;
  char tag[10] = {0};
  struct gadgetStar_header header;
  int64_t total_particles, skip, skip2, halo_particles, i;

  input = check_fopen(filename, "rb");
  gadgetStar2_detect_filetype(input, filename);

#define gadgetStar_variant_block(a)		\
  if (GADGET_VARIANT) { \
  ...
  gadgetStar_variant_block("HEAD");
  fread_fortran(&header, GADGET_HEADER_SIZE, 1, input, SWAP_ENDIANNESS);
  gadgetStar2_extract_header_info(&header);

  total_particles = 0;
  for (i=0; i<6; i++) total_particles += header.num_particles[i];

  if (1) {
    for (skip=0,i=0; i<GSPT; i++) skip += header.num_particles[i];
    halo_particles = header.num_particles[GSPT];
  ...
  *p = (struct particle *)check_realloc(*p, ((*num_p)+halo_particles)*sizeof(struct particle), "Allocating particles.");
  ...
  gadgetStar_variant_block("POS");
  gadgetStar2_read_stride(input, *num_p, halo_particles, 3, sizeof(float), 
    *p, (char *)&(p[0][0].pos[0])-(char*)(p[0]), skip, skip2, filename);

  gadgetStar_variant_block("VEL");
  gadgetStar2_read_stride(input, *num_p, halo_particles, 3, sizeof(float), 
    *p, (char *)&(p[0][0].pos[3])-(char*)(p[0]), skip, skip2, filename);

  gadgetStar_variant_block("ID");
  gadgetStar2_read_stride(input, *num_p, halo_particles, 1, GADGET_ID_BYTES, 
    *p, (char *)&(p[0][0].id)-(char*)(p[0]), skip, skip2, filename);

  gadgetStar2_rescale_particles(*p, *num_p, halo_particles);
